export default function createAutoDlComfyUiPlugin() {
  ...
  const normalizeResolution = (value) => {
    const requested = String(value || "").replace(/p$/i, "");
    return ["480", "768", "1080"].includes(requested) ? requested : "768";
  };
  const normalizeSeconds = (value) => {
    const seconds = Math.round(Number(value));
    return String(Number.isFinite(seconds) ? Math.max(1, Math.min(10, seconds)) : 5);
  };
...
return await poll(
  ...
  { intervalMs: 3000, timeoutMs: 600000 },
      ...
      settings: {
        resolutions: [{ value: "480", label: "480p" }, { value: "768", label: "768p" }, { value: "1080", label: "1080p" }],
        seconds: [1, 3, 5, 6, 8, 10], minSeconds: 1, maxSeconds: 10,
        sizeMode: "dimensions", allowCustomResolution: true,
        normalizeResolution, normalizeSeconds, normalizeSize,
        resolutionLabel: (value) => `${normalizeResolution(value)}p`,
      },
    }],
  };
}
