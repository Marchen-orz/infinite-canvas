...
import { getVideoModelPluginTemplates, type ModelPluginTemplate } from "@/services/api/model-plugins";
...
export function getPluginVariables(): PluginVariable[] {
    return [
        ...
        { name: "audios", type: "string[]", desc: i18n.t("modelPlugin.variables.audios"), capabilities: ["video"] },
        { name: "videos", type: "string[]", desc: "参考视频 Data URL 数组", capabilities: ["video"] },
...
export type PluginTemplate = ModelPluginTemplate;
...
return (data.candidates || [])
  ...
  .map((img) => \`data:\${img.mimeType || img.mime_type || "image/png"};base64,\${img.data}\`);`,
        },
        {
            label: "ComfyUI 图片工作流",
            script: [
                "// ComfyUI 生图 / 改图：images 为空时走文生图，否则使用第一张图作为参考图。",
                "// params.size、params.count、params.negativePrompt、params.denoise、params.upscale、params.upscaleFactor",
                "const workflowParams = {",
                '  negative_prompt: params.negativePrompt || "",',
                "  denoise: Number(params.denoise ?? 0.75),",
                "  upscale: params.upscale !== false,",
                "  upscale_factor: Number(params.upscaleFactor || 2),",
                "};",
                "",
                "if (images.length === 0) {",
                "  const data = await request({",
                ...
                '    url: baseUrl + "/v1/images/generations",',
                '    headers: { "Content-Type": "application/json", Authorization: "Bearer " + apiKey },',
                '    data: { model, prompt, n: params.count, size: params.size, response_format: "b64_json", ...workflowParams },',
                "  });",
                '  return (data.data || []).map((item) => item.b64_json ? "data:image/png;base64," + item.b64_json : item.url);',
                "}",
                "",
                ...
                'form.set("model", model);',
                'form.set("prompt", prompt);',
                'form.set("n", String(params.count || 1));',
                'form.set("size", params.size);',
                'form.set("response_format", "b64_json");',
                'form.set("negative_prompt", workflowParams.negative_prompt);',
                'form.set("denoise", String(workflowParams.denoise));',
                'form.set("upscale", String(workflowParams.upscale));',
                'form.set("upscale_factor", String(workflowParams.upscale_factor));',
                "const referenceBlob = await (await fetch(images[0])).blob();",
