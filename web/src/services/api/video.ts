...
import { resolveVideoModelPlugin } from "./model-plugins";
...
type RequestOptions = { signal?: AbortSignal; audioReferences?: ReferenceAudio[]; videoReferences?: ReferenceVideo[] };
...
async function createPluginVideoTask(config: AiConfig, model: string, script: string, prompt: string, references: ReferenceImage[], options?: RequestOptions): Promise<VideoGenerationTask> {
    ...
    const audios = await Promise.all((options?.audioReferences || []).slice(0, 3).map((audio) => mediaToDataUrl(audio)));
    const videos = await Promise.all((options?.videoReferences || []).slice(0, 9).map((video) => mediaToDataUrl(video)));
    const pluginSettings = resolveVideoModelPlugin(script)?.settings;
    const result = videoPluginResult(
        await runModelPlugin({
            ...
            videos,
            params: {
                seconds: pluginSettings?.normalizeSeconds(config.videoSeconds) || normalizeVideoSeconds(config.videoSeconds),
                size: pluginSettings?.normalizeSize(config.size) || normalizeVideoSize(config.size),
                resolution: pluginSettings?.normalizeResolution(config.vquality) || normalizeVideoResolution(config.vquality),
                ratio: pluginSettings?.normalizeSize(config.size) || config.size,
                // Keep URL arrays aligned with the Data URL arrays so provider plugins can
                // prefer a public URL and fall back to the local Data URL per asset.
                refImageUrls: references.map((reference) => /^https?:\/\//i.test(reference.url || "") ? reference.url : ""),
                refVideoUrls: (options?.videoReferences || []).slice(0, 9).map((reference) => /^https?:\/\//i.test(reference.url || "") ? reference.url : ""),
                refAudioUrls: (options?.audioReferences || []).slice(0, 3).map((reference) => /^https?:\/\//i.test(reference.url || "") ? reference.url : ""),
                refImagesDataUrls: refs,
                refAudiosDataUrls: audios,
                refVideosDataUrls: videos,
                generateAudio: boolConfig(config.videoGenerateAudio, true),
                watermark: boolConfig(config.videoWatermark, false),
                useContextIr: boolConfig(config.videoUseContextIr, false),
                skillId: config.videoSkillId.trim(),
                seed: /^\d+$/.test(config.videoSeed.trim()) && Number(config.videoSeed) > 0 ? config.videoSeed.trim() : "random",
            },
...
async function mediaToDataUrl(media: ReferenceAudio | ReferenceVideo): Promise<string> {
    const source = await resolveMediaUrl(media.storageKey, media.url || "");
    if (!source) return "";
