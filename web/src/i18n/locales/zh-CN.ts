export default {
    ...
    config: {
        ...
        modelSelect: {
            ...
            add: "增加模型",
            fetch: "拉取模型列表",
            description: "如果上游不提供 OpenAI /models 模型列表接口，请在这里手动增加模型 ID；随后可在渠道模型中设置显示名称。",
        ...
        webdav: {
            title: "WebDAV 同步",
            description: "同步全部持久化业务数据：画布、资产、生成记录、本地媒体、AI 配置（含 API Key）、界面设置、提示词源、节点插件、插件私有数据及提示词缓存。当前 WebDAV 连接信息和运行时会话不会同步；浏览器会直接连接 WebDAV 服务。",
            lastSynced: "上次同步 {{time}}",
            neverSynced: "尚未同步",
            url: "WebDAV 地址",
            directory: "远程目录",
            directoryDescription: "会在该目录下分业务目录保存，每个目录包含 {{manifest}}，有媒体文件时另存于 files/",
            username: "用户名",
            password: "密码 / 应用密码",
            test: "测试连接",
            syncing: "同步中",
