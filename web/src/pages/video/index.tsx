...
import { boolConfig, modelOptionLabel, resolveModelScript, useConfigStore, useEffectiveConfig, type AiConfig } from "@/stores/use-config-store";
...
export default function VideoPage() {
    ...
    const previewGenerationLog = (log: GenerationLog) => {
        ...
        if (log.config.videoUseContextIr) updateConfig("videoUseContextIr", log.config.videoUseContextIr);
        if (log.config.videoSkillId !== undefined) updateConfig("videoSkillId", log.config.videoSkillId);
...
function buildVideoConfig(config: AiConfig, model: string): AiConfig {
    const pluginSettings = resolveVideoModelPlugin(resolveModelScript(config, model))?.settings;
    return {
        ...config,
        model,
        videoModel: model,
        size: pluginSettings?.normalizeSize(config.size) || normalizeVideoSize(config.size),
        videoSeconds: pluginSettings?.normalizeSeconds(config.videoSeconds) || normalizeVideoSeconds(config.videoSeconds),
        vquality: pluginSettings?.normalizeResolution(config.vquality) || normalizeResolution(config.vquality),
