...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    return (
        ...
        >
            ...
            {showPanel && !isGroup && renderPanel ? <div className="absolute left-1/2 top-full z-[70] w-max max-w-[calc(100vw-24px)] -translate-x-1/2 pt-4">{renderPanel(data)}</div> : null}
        </div>
    );
});
...
function ConnectionHandleDot({ side, visible, onMouseDown }: { side: "left" | "right"; visible: boolean; onMouseDown: (event: React.MouseEvent) => void }) {
    ...
    return (
        <div
            className={`absolute top-1/2 z-30 flex size-12 -translate-y-1/2 cursor-pointer items-center justify-center transition-opacity duration-150 ${
