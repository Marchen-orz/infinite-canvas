...
export const imageScaleOptions = imageAspectOptions;

type ImageSettingsPanelProps = {
    config: AiConfig;
    model?: string;
    onConfigChange: (key: "quality" | "size" | "count" | "background" | "negativePrompt" | "denoise" | "upscale" | "upscaleFactor", value: string) => void;
    theme: CanvasTheme;
    showTitle?: boolean;
    className?: string;
    maxCount?: number;
...
export function ImageSettingsPanel({ config, model, onConfigChange, theme, showTitle = true, className = "w-[320px] space-y-4 rounded-2xl px-1 py-0.5", maxCount = 15, quickCount = 10 }: ImageSettingsPanelProps) {
    ...
    const modelScript = resolveModelScript(config, model || config.model || config.imageModel);
    const isComfyUi = modelScript.includes("ComfyUI") || (modelScript.includes("const workflowParams") && modelScript.includes("negative_prompt"));
    const denoise = Math.min(1, Math.max(0, Number.isFinite(Number(config.denoise)) ? Number(config.denoise) : 0.75));
    const upscale = config.upscale !== "false";
    const upscaleFactor = Math.max(1, Number(config.upscaleFactor) || 2);
            ...
            >
                ...
                {isComfyUi ? (
                    <div className="space-y-3 rounded-lg border p-3" style={{ borderColor: theme.node.stroke }}>
                        <SettingTitle color={theme.node.muted}>{t("settingsPanels.image.comfyui")}</SettingTitle>
                        <label className="block">
                            <span className="mb-1.5 block text-xs font-medium" style={{ color: theme.node.muted }}>
                                {t("settingsPanels.image.negativePrompt")}
                            </span>
                            <textarea
                                value={config.negativePrompt || ""}
                                rows={3}
                                className="w-full resize-y rounded-md border bg-transparent px-2.5 py-2 text-sm outline-none"
                        ...
                        <label className="block">
                            <span className="mb-1.5 flex items-center justify-between gap-3 text-xs font-medium" style={{ color: theme.node.muted }}>
                                <span>{t("settingsPanels.image.denoise")}</span>
                            ...
                            <input
                                ...
                                onChange={(event) => onConfigChange("denoise", event.target.value)}
                                onMouseDown={(event) => event.stopPropagation()}
                            />
                            <span className="mt-1 block text-[11px]" style={{ color: theme.node.muted }}>
                                {t("settingsPanels.image.denoiseHint")}
                            </span>
                        </label>
                        <div className="flex items-center justify-between gap-3">
                            <SettingTitle color={theme.node.muted}>{t("settingsPanels.image.upscale")}</SettingTitle>
                            <span onMouseDown={(event) => event.stopPropagation()}>
                                <Switch size="small" checked={upscale} onChange={(checked) => onConfigChange("upscale", String(checked))} />
                            </span>
                        </div>
                        <label className="block">
                            <span className="mb-1.5 block text-xs font-medium" style={{ color: theme.node.muted }}>
                                {t("settingsPanels.image.upscaleFactor")}
                            </span>
                            <input
                                type="number"
                                min={1}
                                ...
                                style={{ borderColor: theme.node.stroke, color: theme.node.text }}
                                onChange={(event) => onConfigChange("upscaleFactor", event.target.value)}
                                onMouseDown={(event) => event.stopPropagation()}
                            />
                        </label>
                    </div>
