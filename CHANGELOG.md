...
+ [修复] 提高剪辑台节目监看高度并让面板垂直滚动，竖屏素材会完整等比显示而不会被截断。
+ [修复] 修正剪辑台素材箱素材项被网格拉伸的问题，素材卡片现在保持固定紧凑高度。
+ [调整] 音视频剪辑台素材箱仅显示与剪辑台直接连线的图片、音频和视频，新增图片裁剪、裁剪素材时间轴排布及右键创建图片节点，并压缩素材项尺寸。
+ [优化] 音视频剪辑台改用与暗色画布一致的暖黑中性色，限制预览监看尺寸，并让未展开的插件节点内容铺满卡片。
+ [调整] 已生成的图片和视频支持在原节点使用最新提示词与模型参数重新生成，不再额外创建结果节点。
+ [修复] 优云智算 MiniMax H3 插件处理缺少 MIME 的本地 Data URL，优先使用公网素材地址并兼容已保存的 1.1.0 脚本迁移。
+ [修复] 优云智算 MiniMax H3 插件兼容嵌套或字符串任务响应，自动升级旧脚本并展示接口返回的真实错误。
...
- [调整] 将剪辑台时间轴改为真实可编辑双轨：素材可拖入、移动排列、帧级裁切、播放头定位、删除和按时间轴顺序导出。
